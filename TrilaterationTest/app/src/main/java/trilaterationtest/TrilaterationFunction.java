...
public class TrilaterationFunction implements MultivariateJacobianFunction {
	...
	protected final double sigmaDistances[];
	protected final double sigmaPositions[];

	public TrilaterationFunction(double positions[][], double distances[], double[] sigmaPositions, double[] sigmaDistances) {
        ...
        this.sigmaPositions = new double[distances.length];
        this.sigmaDistances = new double[distances.length];
		// bound distances to strictly positive domain
		for (int i = 0; i < distances.length; i++) {
			distances[i] = Math.max(distances[i], epsilon);
            this.sigmaPositions[i] =  Math.max(this.sigmaPositions[i], epsilon);
            this.sigmaDistances[i] =  Math.max(this.sigmaDistances[i], epsilon);
		}
